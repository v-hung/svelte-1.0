    ...
    <div class="rounded-xl px-4 py-3 bg-white">
      ...
      <div class="flex flex-col space-y-1 -mx-3 mt-2">
        <button 
          ...
          {question.answer == question.option_a ? 'active' : ''}"
          on:click={() => question.answer = question.option_a}
        >
          <span class="grid place-items-center w-6 h-6 rounded-full bg-gray-200 text-sm">A</span>
          <span class="text-sm">{question.option_a}</span>
        </button>

        <button 
          class="flex items-center space-x-2 px-3 py-1 hover:bg-gray-100 rounded-lg 
          {question.answer == question.option_b ? 'active' : ''}"
          on:click={() => question.answer = question.option_b}
        >
          <span class="grid place-items-center w-6 h-6 rounded-full bg-gray-200 text-sm">B</span>
          <span class="text-sm">{question.option_b}</span>
        </button>

        <button 
          class="flex items-center space-x-2 px-3 py-1 hover:bg-gray-100 rounded-lg 
          {question.answer == question.option_c ? 'active' : ''}"
          on:click={() => question.answer = question.option_c}
        >
          <span class="grid place-items-center w-6 h-6 rounded-full bg-gray-200 text-sm">C</span>
          <span class="text-sm">{question.option_c}</span>
        </button>

        <button 
          class="flex items-center space-x-2 px-3 py-1 hover:bg-gray-100 rounded-lg 
          {question.answer == question.option_d ? 'active' : ''}"
          on:click={() => question.answer = question.option_d}
        >
          <span class="grid place-items-center w-6 h-6 rounded-full bg-gray-200 text-sm">D</span>
          <span class="text-sm">{question.option_d}</span>
        </button>
      </div>
    </div>
  {/each}
