<script lang="ts">
  ...
  const toggleAll = () => {
    if (check_all) {
      ...
      checked_products = data.products.map((v,i) => v = i)
    }
  }
</script>
...
<section class="flex justify-between items-end">
  <div>
    ...
    <p class="text-gray-600">{data.products.length} entries found</p>
  </div>
  ...
  {#if data.products.length > 0}
    <div class="w-full overflow-x-auto border rounded overflow-hidden shadow-sm px-6 bg-white">
      <table>
        ...
        <tbody>
          {#each data.products as a,i}
            <tr class="cursor-pointer transition-colors hover:border-orange-600">
              <td>
                <input type="checkbox" name="product" bind:group={checked_products} value="{i}">
              </td>
              <td>
                4
              </td>
